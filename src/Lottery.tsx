...
function Term({title, desc, register, errors}: any) {
  return (
    <Field name={`${title}.accept`} errors={errors}>
      <label htmlFor={title}>{title}</label>
      <label htmlFor={title}>{desc}</label>
      <span className="radio-group">
        <input ref={register({validate: (value: any) => value === 'accept'})} type="radio" name={`${title}.accept`} value="accept"></input>
        <label htmlFor="accept">I accept</label>
        <input ref={register} type="radio" name={`${title}.decline`} value="decline"></input>
